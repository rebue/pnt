...
public class PntLogTests {
    ...
    public void testCrud() throws IOException, ReflectiveOperationException {
        final PntLogMo mo = (PntLogMo) MockDataUtils.newRandomPojo(new PntLogMo().getClass());
        mo.setId(null);
        System.out.println("添加积分日志的参数为：" + mo);
        final String addResult = OkhttpUtils.postByJsonParams(hostUrl + "/pnt/log", mo);
        ...
        final String listResult = OkhttpUtils.get(hostUrl + "/pnt/log?pageNum=1&pageSize=5");
        System.out.println("查询积分日志的返回值为：" + listResult);
        System.out.println("获取单个积分日志的参数为：" + mo.getId());
        final String getByIdResult = OkhttpUtils.get(hostUrl + "/pnt/log/getbyid?id=" + mo.getId());
        System.out.println("获取单个积分日志的返回值为：" + getByIdResult);
        System.out.println("修改积分日志的参数为：" + mo);
        final String modifyResult = OkhttpUtils.putByJsonParams(hostUrl + "/pnt/log", mo);
        ...
        final String deleteResult = OkhttpUtils.delete(hostUrl + "/pnt/log?id=" + mo.getId());
