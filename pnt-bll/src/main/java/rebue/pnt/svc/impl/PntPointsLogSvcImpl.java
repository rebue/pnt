...
@Service
public class PntPointsLogSvcImpl
		extends BaseSvcImpl<java.lang.Long, PntPointsLogJo, PntPointsLogDao, PntPointsLogMo, PntPointsLogMapper>
		implements PntPointsLogSvc {
	...
	public Ro addPointTrade(AddPointTradeTo to) {
		...
		Ro ro = new Ro();
		if (to.getAccountId() == null || to.getOrderId() == null || to.getPointsLogType() == null
				|| to.getChangedPoints() == null || to.getChangedTitile() == null
				|| to.getModifiedTimestamp() == null) {
		...
		case ORDER_SETTLE: // XXX 订单结算（积分+）
			// 新当前积分 = 修改的积分 + 旧的当前积分
			newPoints = to.getChangedPoints() + accountMo.getPoints();
			break;
		case ORDER_RETURN: // XXX 订单退货（积分-）
		case VPAY_WITHDRAW: // XXX v支付提现（积分-）
			// 新当前积分 = 旧的当前积分 - 修改的积分
			newPoints = accountMo.getPoints() - to.getChangedPoints();
			break;
		default:
