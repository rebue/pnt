...
public class PntIncomeLogSvcImpl
	...
	public void executePointIncomeTask() {
		...
		List<PntAccountMo> accountList = pntAccountSvc.listAll();
		_log.info("执行积分收益任务查询所有积分账号信息的返回值为：{}", accountList);
		for (PntAccountMo accountMo : accountList) {
			if (accountMo.getIsLocked()) {
				_log.error("执行积分收益任务时发现账号id为：{}的账号已被锁定", accountMo.getId());
				continue;
			}
			BigDecimal pointAfterChanged = pntPointLogSvc.getPointAfterChangedByAccountId(accountMo.getId());
			...
			BigDecimal changedIncome = pointAfterChanged.multiply(dailyInterestRate);
			...
			Ro addIncomeTradeRo = thisSvc.addIncomeTrade(addIncomeTradeTo);
