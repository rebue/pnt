...
public interface PntAccountMapper extends MybatisBaseMapper<PntAccountMo, Long> {
    ...
    "SELECT " + //
            ...
            + " and REG_TIME < #{yesterdayRegTime,jdbcType=TIMESTAMP} " + " LIMIT #{fetchCount}")
    List<PntAccountMo> listToCalcDayIncome(@Param("yesterday") java.sql.Date yesterday,
            @Param("fetchCount") int fetchCount, @Param("yesterdayRegTime") java.util.Date yesterdayRegTime);
    ...
    int updatePoint(@Param("id") Long id, @Param("newPoint") BigDecimal newPoint,
            @Param("oldPoint") BigDecimal oldPoint, @Param("newModifiedTimestamp") Long newModifiedTimestamp,
    ...
    List<PntAccountMo> selectPntAccountByLimitCount(@Param("pageNum") Integer pageNum,
     ...
     * @return
     */
    @Select("SELECT * FROM pnt.PNT_ACCOUNT where point !=0")
    List<PntAccountMo> selectPntAccountByNotZero();
}
