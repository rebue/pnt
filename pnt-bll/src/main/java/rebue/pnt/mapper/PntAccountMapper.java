...
public interface PntAccountMapper extends MybatisBaseMapper<PntAccountMo, Long> {
	 ...
	 * @return
	 */
	@Update("update PNT_ACCOUNT set POINTS=#{newPoints,jdbcType=DECIMAL}, MODIFIED_TIMESTAMP=#{newModifiedTimestamp,jdbcType=BIGINT} where ID=#{id,jdbcType=BIGINT} and POINTS=#{oldPoints,jdbcType=DECIMAL} and MODIFIED_TIMESTAMP=#{oldModifiedTimestamp,jdbcType=BIGINT}")
	int updatePoints(@Param("id") Long id, @Param("newPoints") BigDecimal newPoints,
			@Param("oldPoints") BigDecimal oldPoints, @Param("newModifiedTimestamp") Long newModifiedTimestamp,
	...
	int updateIncome(@Param("id") Long id, @Param("newIncome") BigDecimal newIncome,
			@Param("oldIncome") BigDecimal oldIncome, @Param("newTotalIncome") BigDecimal newTotalIncome,
			@Param("oldTotalIncome") BigDecimal oldTotalIncome,
